...
ignoreIDs = [classID for classID, name in model.names.items() if name in ignoreClasses]
    ...
    while True:
        ...
        if key == ord("u"):
            ...
            for _ in range(n):
                ...
                if results[0].boxes.conf.numel() > 0:  # Ensure at least one object is detected
            ...
            if len(frameBuffer) > 0:
                ...
                bestBoxes = bestResult.boxes.xyxy  # ✅ Fixed: Get bounding boxes
                bestClassIDs = bestResult.boxes.cls  # ✅ Fixed: Get class IDs

                processedFrame = bestFrame.copy()

                for box, classID in zip(bestBoxes, bestClassIDs):  # ✅ Fixed: Loop over bestBoxes
